...
const PersonaSettings = () => {
  ...
  // Stable fetcher (accepts AbortSignal)
  ...
  // Fetch when persona changes or when you explicitly refresh
  useEffect(() => {
  ...
  const handleAddCredential = async () => {
    ...
    try {
      ...
      if (data?.status === "success" || data?.status === "noop") {
        ...
        // Trigger a one-time refresh so the new cred shows immediately
        setRefreshKey((k) => k + 1);
      } else {
